...
## 📌 Mantenimiento Obligatorio de DOCS.md

Cada vez que hagas un cambio debe quedar descrito el cambio en el archivo `DOCS.md` de forma secuencial y ordenada. Si es que se deshace un cambio entonces se debe deshacer también lo descrito en el archivo `DOCS.md`, este archivo va a ser una hoja de vida del proyecto para siempre estar al día respecto a los cambios que se hacen y así poder en el futuro leerlo y entender rápidamente lo que estábamos haciendo.

En el archivo `DOCS.md` siempre debe registrarse:
1. **Fecha**
2. **Nombre de la rama de Git** (cada rama Git va a tener su propio `DOCS.md`).
3. **Log numerado secuencialmente** por cada cambio, el cual DEBE contener:
   - Información y justificación del **porqué** se hace lo que se está haciendo.
   - Una breve descripción de los archivos modificados, creados o eliminados y carpetas agregadas.
...
- [x] Inicializar un proyecto de Payload CMS (v3.0+) configurado con TypeScript y SQLite (`cms/cms.db`).
